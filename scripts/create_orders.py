...
# Shipper.Id
c.execute("select distinct id from Shippers")
shippers = [row[0] for row in c.fetchall()]

# Customer.Id
c.execute("select distinct id from Customers")
...
# Product.Id
c.execute("select distinct id, UnitPrice from Products")
products = [(row[0], row[1]) for row in c.fetchall()]

# Order.Id
c.execute("select distinct id from [Orders] where Freight = 0.00")
...
for order in orders:
  ...
  for x in range(randint(1,len(products))):
    sql = 'INSERT INTO OrderDetails (Id, OrderId, ProductId, UnitPrice, Quantity, Discount) VALUES (?, ?, ?, ?, ?, ?)'
    control = 1
    while control:
      product = rc(products)
...
for order in orders:
  c.execute("update Orders set Freight=? where Id=?", (order[0], order[1]))
